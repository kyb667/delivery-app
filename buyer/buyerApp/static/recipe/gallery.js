    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var i = 0; i < len; i++) {
          ...
          show_html +=
            '<li class="one_quarter show_recipedetail ' +
            first +
            '" name="' +
            info.name +
            '">' +
...
$(document).on("click", "#direct_search_btn", function () {
  ...
  if (direct_search_check % 2 == 1) {
    html =
      '<header> <address> By <a id="direct_search_btn"> 직접검색 <i id="direct_search_btn_css" class="fa fa-chevron-up"></i> </a> </address> </header>';
    html +=
      '<div> <fieldset><input type="text" id="direct_search_input" placeholder="음식 이름을 입력해주세요" onkeyup="select_search_word(this)"><ul id="search_word_list"></ul></fieldset></div>';
  } else {
    html =
      '<header> <address> By <a id="direct_search_btn"> 직접검색 <i id="direct_search_btn_css" class="fa fa-chevron-down"></i> </a> </address> </header>';
  }
...
$(document).on("click", "#condition_search_btn", function () {
  ...
  if (condition_search_check % 2 == 1) {
    html =
      '<header> <address> By <a id="condition_search_btn"> 조건검색 <i id="condition_search_btn_css" class="fa fa-chevron-up"></i> </a></address> </header>';
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var i = 0; i < result.length; i++) {
          info = result[i];
          html +=
            '<li style="background-color: white;"> <a class="select_recipe_group" id="' +
            info.fooddetail +
            '">' +
            info.fooddetail +
            " </a> </li>";
    ...
    html =
      '<header> <address> By <a id="condition_search_btn"> 조건검색 <i id="condition_search_btn_css" class="fa fa-chevron-down"></i> </a></address> </header>';
    showHtml.innerHTML = html;
  }
});
//조건검색 그룹 표시
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
          info = data[i];
          html +=
            '<li style="background-color:#F7F7F7" class="show_recipedetail" name="' +
            info.foodname +
            '"><a>' +
            info.foodname +
            "</a></li>";
